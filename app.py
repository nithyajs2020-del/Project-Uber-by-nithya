...
with open('trained_model_uber.pkl', 'rb') as file:
    model = pickle.load(file)

# Load the scalers
with open('x_scaler_uber.pkl', 'rb') as file:
    x_scaler = pickle.load(file)
with open('y_scaler_uber.pkl', 'rb') as file:
    y_scaler = pickle.load(file)

# Load the one-hot encoded column names
with open('one_hot_encoded_columns_uber.pkl', 'rb') as file:
...
with col2:
    ...
    if st.button('Predict Fare'):
        ...
        # Ensure the feature order for scaling matches the training order
        features_to_scale = ['distance', 'Day_of_week', 'Hour', 'Month', 'Year']
        new_data_for_x_scaling = new_data[features_to_scale]
        new_data_scaled_x_features_array = x_scaler.transform(new_data_for_x_scaling.values)
        scaled_x_df = pd.DataFrame(new_data_scaled_x_features_array, columns=features_to_scale)

        # Reconstruct the final input for the model, ensuring the correct order of features
        # The order should match the features used for x_train:
        # ['distance', 'is_weekend', 'Day_of_week', 'Hour','Month','Year','Uber Go','Uber XL']
        final_model_input_df = pd.DataFrame()
        final_model_input_df['distance'] = scaled_x_df['distance']
        final_model_input_df['is_weekend'] = new_data['is_weekend']
        final_model_input_df['Day_of_week'] = scaled_x_df['Day_of_week']
        final_model_input_df['Hour'] = scaled_x_df['Hour']
        final_model_input_df['Month'] = scaled_x_df['Month']
        final_model_input_df['Year'] = scaled_x_df['Year']
        final_model_input_df['Uber Go'] = new_data['Uber Go']
        final_model_input_df['Uber XL'] = new_data['Uber XL']

        final_model_input_array = final_model_input_df.values # Convert to numpy array for model prediction
        
        # Make prediction
        prediction_scaled = model.predict(final_model_input_array)
